...
#else
    this->setGeometry(QStyle::alignedRect(Qt::LeftToRight, Qt::AlignCenter, this->size(), QGuiApplication::primaryScreen()->geometry()));
#endif
...
void DelayCut::execCLI(int argc)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (parameter == "-fps" && i < (argc - 1))
        {
            ...
            {
                if (sscanf(fpsParam.toLatin1().constData(), "%lf", &fps) != 1)
                {
                    fprintf(stderr, "No valid fps value specified.\n");
                    exit(EXIT_FAILURE);
                }
        ...
        {
            i++;
            if (sscanf(args.at(i).toLatin1().constData(), "%lf", &startDelay) != 1)
            {
        ...
        {
            i++;
            if (sscanf(args.at(i).toLatin1().constData(), "%lf", &endDelay) != 1)
            {
        ...
        {
            i++;
            if (sscanf(args.at(i).toLatin1().constData(), "%lf", &startCut) != 1)
            {
        ...
        {
            i++;
            if (sscanf(args.at(i).toLatin1().constData(), "%lf", &endCut) != 1)
            {
